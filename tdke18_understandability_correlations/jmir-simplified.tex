...
In the experiments, we used Pearson, Kendall and Spearman correlations to compare the understandability assessments of human assessors with estimations obtained by the considered automated approaches, under all combinations of pipelines and heuristics \todo{It's a bit difficult to understand this before the methods have been explained.}. Pearson correlation is used to calculate the strength of the linear relationship between two variables, while Kendall and Spearman measure the rank correlations between the variables. We opted to report all three correlation coefficients to allow for a thorough comparison to other work, as they are equally used in the literature. 

For the retrieval experiments \todo{Again, difficult to follow without knowing what the experiments are. I think especially difficult for people outside IR.}, we used evaluation measures that rely on both (topical) relevance and understandability. 
...
the sentiment polarity \todo{include citation} and the ratio of words found in English vocabularies. The Python package NLTK~\cite{nltk} was employed for sentiment analysis, POS tagging and entity recognition. The GNU Aspell~\cite{aspell} dictionary was used as a standard English vocabulary and a stop word list was built by merging those of Indri~\cite{indri} and Terrier~\cite{terrier}. 
...
Unless explicitly stated otherwise, we ignored out of vocabulary words in the feature calculations \todo{How were the features calculated?} .
...
\todo{What did you do with these statistics to calculate readability? I'm a bit confused by this section.}
...
\begin{itemize}[leftmargin=*]
    \item Medical Reddit (label 1 \todo{What are these labels?}): Documents in this corpus are expected to be written in a colloquial style, and thus the easiest to understand. All the conversations are in fact explicitly directed to assist inexpert health consumers;
...
Note that for XGB, for assessed documents we used 10-fold cross validation, training XGB on 90\% of the data, and used its predictions for the remaining 10\%. For unassessed documents, we trained XGB on all assessed data and applied this model to generate predictions. Different machine learning methods and feature selection schemes were experimented with; results are available in the appendix. XGB was selected because its results were the best, although other methods followed similar trends \todo{Very vague.}.
