...
\begin{figure}[t!]
   ...
   \includegraphics[width=.45\textwidth]{graphics/cumdist}
    \caption{Cumulative distribution of Dale-Chall Index (measuring the number of years at school to understand a document) for documents retrieved by different methods. The scores distribution of documents retrieved by Health on Net (HON) is similar to the baseline method used in our experiments (BM25 Bl.). Our best method (XGB) re-ranks documents retrieved by the baseline to provide more understandable documents at the top, which leads to a distribution closer to an oracle method. Note that the average US resident reads at or below an 8th grade level~\cite{wallace04,davis04,stossel12} (dashed line).}
   \label{fig:dist}
\end{figure}
