...
\begin{figure}[t!]
   ...
   \includegraphics[width=.45\textwidth]{graphics/cumdist}
    \caption{Distribution of Dale-Chall Index (DCI) of search results. DLI  measures the number of years of schooling required to understand a document. The average US resident reads at or below an 8th grade level~\cite{wallace04,davis04,stossel12} (dashed line), which is the level suggested by NIH for health information on the Web. The distribution for HON is similar to that of the baseline used in this paper (BM25 Bl.). Our best method (XGB) re-ranks documents to provide more understandable results; its distribution is similar to that of an ``Oracle'' system.}
   \label{fig:dist}
\end{figure}
